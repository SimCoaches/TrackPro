...
Name "TrackPro v1.2.2"
OutFile "TrackPro_Setup_v1.2.2.exe"
...
Section "Prerequisites"
    ${If} $0 != "RESUME"
        ...
        File "installer_temp\dist\TrackPro_v1.2.2.exe"
        
        ; Verify the file was extracted correctly
        ${IfNot} ${FileExists} "$TEMP\TrackPro\app\TrackPro_v1.2.2.exe"
            MessageBox MB_OK|MB_ICONSTOP "Failed to extract TrackPro_v1.2.2.exe to temporary directory!"
            Abort "Installation failed: Could not extract TrackPro_v1.2.2.exe"
        ${EndIf}
        ...
        StrCpy $DEBUG_MSG "Copying from: $TEMP\TrackPro\app\TrackPro_v1.2.2.exe to $PROGRAMFILES64\TrackPro"
        DetailPrint $DEBUG_MSG
        
        ; Check if destination file already exists and try to remove it
        ${If} ${FileExists} "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.2.exe"
            DetailPrint "Removing existing installation file..."
            Delete "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.2.exe"
        ${EndIf}
        
        ; Clear any previous errors and copy the file
        ClearErrors
        CopyFiles /SILENT "$TEMP\TrackPro\app\TrackPro_v1.2.2.exe" "$PROGRAMFILES64\TrackPro"
        
        ; Check for copy errors
        ${If} ${Errors}
            MessageBox MB_OK|MB_ICONSTOP "Failed to copy TrackPro_v1.2.2.exe to installation directory! Please ensure you have administrator privileges and try again."
            Abort "Installation failed: Could not copy TrackPro_v1.2.2.exe"
        ${EndIf}
        
        ; Verify TrackPro.exe exists in the destination
        ${IfNot} ${FileExists} "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.2.exe"
            MessageBox MB_OK|MB_ICONSTOP "Failed to verify TrackPro_v1.2.2.exe in installation directory!"
            Abort "Installation failed: Could not verify TrackPro_v1.2.2.exe"
        ${EndIf}
            
        ; Create shortcuts with verification
        DetailPrint "Creating shortcuts..."
        CreateDirectory "$SMPROGRAMS\TrackPro"
        CreateShortCut "$SMPROGRAMS\TrackPro\TrackPro v1.2.2.lnk" "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.2.exe"
        CreateShortCut "$DESKTOP\TrackPro v1.2.2.lnk" "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.2.exe"
        ...
        MessageBox MB_OK|MB_ICONINFORMATION \
            "TrackPro v1.2.2 has been installed to:$\n\
            $PROGRAMFILES64\TrackPro\TrackPro_v1.2.2.exe$\n\n\
            Shortcuts have been created:$\n\
            - Start Menu: $SMPROGRAMS\TrackPro\TrackPro v1.2.2.lnk$\n\
            - Desktop: $DESKTOP\TrackPro v1.2.2.lnk$\n\n\
...
Section "MainApplication"
    ...
    ${If} $0 == "RESUME"
        ...
        ; Create shortcuts
        CreateDirectory "$SMPROGRAMS\TrackPro"
        CreateShortCut "$SMPROGRAMS\TrackPro\TrackPro v1.2.2.lnk" "$INSTDIR\TrackPro_v1.2.2.exe"
        CreateShortCut "$DESKTOP\TrackPro v1.2.2.lnk" "$INSTDIR\TrackPro_v1.2.2.exe"
...
Section "Uninstall"
    ...
    Delete "$INSTDIR\TrackPro_v1.2.2.exe"
    RMDir "$INSTDIR"
    
    Delete "$SMPROGRAMS\TrackPro\TrackPro v1.2.2.lnk"
    Delete "$DESKTOP\TrackPro v1.2.2.lnk"
