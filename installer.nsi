...
Name "TrackPro v1.2.4"
OutFile "TrackPro_Setup_v1.2.4.exe"
...
Section "Prerequisites"
    ${If} $0 != "RESUME"
        ...
        File "installer_temp\dist\TrackPro_v1.2.4.exe"
        
        ; Verify the file was extracted correctly
        ${IfNot} ${FileExists} "$TEMP\TrackPro\app\TrackPro_v1.2.4.exe"
            MessageBox MB_OK|MB_ICONSTOP "Failed to extract TrackPro_v1.2.4.exe to temporary directory!"
            Abort "Installation failed: Could not extract TrackPro_v1.2.4.exe"
        ${EndIf}
        ...
        StrCpy $DEBUG_MSG "Copying from: $TEMP\TrackPro\app\TrackPro_v1.2.4.exe to $PROGRAMFILES64\TrackPro"
        DetailPrint $DEBUG_MSG
        
        ; Check if destination file already exists and try to remove it
        ${If} ${FileExists} "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.4.exe"
            DetailPrint "Removing existing installation file..."
            Delete "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.4.exe"
        ${EndIf}
        
        ; Clear any previous errors and copy the file
        ClearErrors
        CopyFiles /SILENT "$TEMP\TrackPro\app\TrackPro_v1.2.4.exe" "$PROGRAMFILES64\TrackPro"
        
        ; Check for copy errors
        ${If} ${Errors}
            MessageBox MB_OK|MB_ICONSTOP "Failed to copy TrackPro_v1.2.4.exe to installation directory! Please ensure you have administrator privileges and try again."
            Abort "Installation failed: Could not copy TrackPro_v1.2.4.exe"
        ${EndIf}
        
        ; Verify TrackPro.exe exists in the destination
        ${IfNot} ${FileExists} "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.4.exe"
            MessageBox MB_OK|MB_ICONSTOP "Failed to verify TrackPro_v1.2.4.exe in installation directory!"
            Abort "Installation failed: Could not verify TrackPro_v1.2.4.exe"
        ${EndIf}
            
        ; Create shortcuts with verification
        DetailPrint "Creating shortcuts..."
        CreateDirectory "$SMPROGRAMS\TrackPro"
        CreateShortCut "$SMPROGRAMS\TrackPro\TrackPro v1.2.4.lnk" "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.4.exe"
        CreateShortCut "$DESKTOP\TrackPro v1.2.4.lnk" "$PROGRAMFILES64\TrackPro\TrackPro_v1.2.4.exe"
        ...
        MessageBox MB_OK|MB_ICONINFORMATION \
            "TrackPro v1.2.4 has been installed to:$\n\
            $PROGRAMFILES64\TrackPro\TrackPro_v1.2.4.exe$\n\n\
            Shortcuts have been created:$\n\
            - Start Menu: $SMPROGRAMS\TrackPro\TrackPro v1.2.4.lnk$\n\
            - Desktop: $DESKTOP\TrackPro v1.2.4.lnk$\n\n\
...
Section "MainApplication"
    ...
    ${If} $0 == "RESUME"
        ...
        ; Create shortcuts
        CreateDirectory "$SMPROGRAMS\TrackPro"
        CreateShortCut "$SMPROGRAMS\TrackPro\TrackPro v1.2.4.lnk" "$INSTDIR\TrackPro_v1.2.4.exe"
        CreateShortCut "$DESKTOP\TrackPro v1.2.4.lnk" "$INSTDIR\TrackPro_v1.2.4.exe"
...
Section "Uninstall"
    ...
    Delete "$INSTDIR\TrackPro_v1.2.4.exe"
    RMDir "$INSTDIR"
    
    Delete "$SMPROGRAMS\TrackPro\TrackPro v1.2.4.lnk"
    Delete "$DESKTOP\TrackPro v1.2.4.lnk"
