...
class RaceCoachWidget(QWidget):
        ...
        if self.iracing_api is not None:
            try:
                ...
                if hasattr(self.iracing_api, 'register_on_connection_changed'):
                    ...
                    # Connect to the new_trackpro_session_created signal for session change notifications
                    if hasattr(self.iracing_api, 'new_trackpro_session_created'):
                        logger.info("Connecting new_trackpro_session_created signal to session refresh handler.")
                        self.iracing_api.new_trackpro_session_created.connect(self._on_new_session_created)
                    else:
                        logger.warning("iRacing API instance does not have new_trackpro_session_created signal.")
                    # ----------------------------- #
    ...
    def _format_lap_display(self, lap_num, lap_time, stint_num=None):
        """Format lap number and time for display in UI, optionally including stint number."""
        ...
        if lap_num == 0:
            display_text = f"Out lap  -  {time_str}"
        else:
            display_text = f"Lap {lap_num}  -  {time_str}"
            
        # Add stint number if provided
        if stint_num is not None:
            return f"Stint {stint_num} - {display_text}"
        else:
            return display_text
        ...
        try:
            ...
            if hasattr(self, 'iracing_api') and self.iracing_api:
                ...
                try:
                    ...
                    # Try to connect and get status - FIXED to use the SimpleIRacingAPI's ir instance
                    if self.iracing_api and hasattr(self.iracing_api, 'ir'):
                        is_connected = is_iracing_available(self.iracing_api.ir)
                        logger.info(f"Direct iRacing connection test result: {is_connected}")
                        
                        # If connected, try to parse data (if needed)
                        if is_connected and hasattr(run_irsdk_parse, '__call__'):
                            parse_result = run_irsdk_parse()
                            logger.info(f"iRacing data parse result: {parse_result}")
                    else:
                        logger.warning("Cannot check iRacing connection: iracing_api or ir not available")
    ...
    def _on_initial_laps_loaded(self, laps_data, message):
        """Slot to handle the loaded laps from the background worker for the initially selected session."""
        logger.info(f"Received {len(laps_data) if laps_data else 0} initial laps. Message: {message}")

        self.left_lap_combo.clear()
        self.right_lap_combo.clear()

        if "Error" in message or not laps_data:
            display_message = message or "No laps found for this session"
            self.left_lap_combo.addItem(display_message, None)
            self.right_lap_combo.addItem(display_message, None)
            self.loading_label.setText(display_message) # Show error/status
            self.clear_all_graphs() # Clear graphs if no laps or error
            return
        ...
        for lap in laps_data:
            ...
            is_valid = lap.get("is_valid", True) # Assume valid if not specified
            stint_num = lap.get("stint_number")

            display_text = self._format_lap_display(lap_num, lap_time, stint_num)
            if not is_valid:
        ...
        # Attempt to select first valid lap for Lap A and second for Lap B (if available)
        ...
        elif self.right_lap_combo.count() > 0: # If only one lap, select it for B too
            self.right_lap_combo.setCurrentIndex(0)
        
        self.loading_label.setText("Laps loaded.") # Update loading label
        # Trigger comparison for initially selected laps
        # self.on_lap_selection_changed() # This will be called by setCurrentIndex if connected
    ...
    def refresh_session_list(self):
        ...
        # Show all sessions, even those without laps yet
    ...
    def refresh_lap_list(self, session_id):
        """Fetch latest laps from Supabase and populate combo boxes, grouped by stints."""
        ...
        try:
            # Include the new columns needed for stint grouping
            columns = "id,lap_number,lap_time,is_valid,session_time_completed,started_on_pit_road,ended_on_pit_road,metadata"
            # Remove the limit to get all laps
            response = main_supabase.client.table("laps").select(columns).eq('session_id', session_id).order('session_time_completed').execute()
        ...
        # Process laps to determine stints
        processed_laps = []
        current_stint = 1
        ...
        for i, lap in enumerate(laps):
            ...
            started_on_pit = lap.get("started_on_pit_road", False)
            
            # Skip laps with invalid times
            if lap_time is None or not isinstance(lap_time, (int, float)) or lap_time <= 0:
                continue
                
            # A new stint starts with the first lap or if the lap started on pit road
            if i == 0 or started_on_pit:
                current_stint += 1 if i > 0 else 0  # Only increment if not first lap
            ...
            processed_laps.append({
                ...
                "metadata": lap.get("metadata", "{}")
            })

        # Success! Update the combo boxes with stint-aware laps
        new_left_index = 0
        new_right_index = 0

        for index, lap in enumerate(processed_laps):
            lap_id = lap.get("id")
            lap_num = lap.get("lap_number", "N/A")
            lap_time = lap.get("lap_time")
            is_valid = lap.get("is_valid", True)
            ...
            # Use the updated lap display formatting function with stint number
            display_text = self._format_lap_display(lap_num, lap_time, stint_num)
        ...
        logger.info(f"Populated lap lists with {len(processed_laps)} laps in {current_stint} stints.")
    ...
    def load_throttle_data(self, lap_id: str):
        ...
        logger.info(f"Loading all channel data for single lap view: {lap_id}")
        
        try:
            # 1. Get Supabase client (used for lap and track details, get_telemetry_points handles its own client)
            client = get_supabase_client()
            if not client:
                logger.error("Cannot load lap/track details: No Supabase client available")
                # Attempt to load telemetry anyway, graphs might lack full context
                # return # Decided to proceed to try and load telemetry points at least
            ...
            if client: # Only try to fetch context if client is available
                ...
                lap_result = client.table("laps").select("session_id").eq("id", lap_id).single().execute()
                    
                if not lap_result.data:
                    logger.warning(f"Lap context for {lap_id} not found")
                    # Not returning, will try to get points anyway
                else:
                    lap_data_for_context = lap_result.data
                
                # 3. Get track information and session context if lap context was found
                    ...
                    try:
                        ...
                        if session_id:
                            ...
                            if session_result.data:
                                ...
                                if track_id:
                                    # Fetch track details using the helper if available, or direct query
                                    if hasattr(self, '_fetch_track_context'):
                                        # Assuming _fetch_track_context also gets track_length
                                        # We need track_length first to pass to _fetch_track_context
                                    ...
                                    else: # Fallback direct query if _fetch_track_context isn't there (it should be)
                                        track_result = client.table("tracks").select("name, length_meters").eq("id", track_id).single().execute()
                                        if track_result.data:
                                            track_length = float(track_result.data.get("length_meters", 0))
                                            track_context = {"name": track_result.data.get("name", "Unknown Track")}
                    except Exception as track_err:
                        logger.warning(f"Error extracting track data: {track_err}")
            else: # No client for context
            ...
            # Remove "LapDist" as it doesn't exist in the DB table; it's calculated later.
            required_columns = ["timestamp", "throttle", "brake", "track_position", "steering", "speed"]
            points_data, message = get_telemetry_points(lap_id, columns=required_columns)
                            
            if points_data is None:
                logger.warning(f"No telemetry data found for lap {lap_id} via get_telemetry_points: {message}")
                if hasattr(self, 'throttle_graph'): self.throttle_graph.update_graph([], track_length, track_context)
                if hasattr(self, 'steering_graph'): self.steering_graph.update_graph([], track_length, track_context)
                if hasattr(self, 'brake_graph'): self.brake_graph.update_graph([], track_length, track_context)
                if hasattr(self, 'speed_graph'): self.speed_graph.update_graph([], track_length, track_context)
                if hasattr(self, 'graph_status_label'): self.graph_status_label.setVisible(False)
                return

            # 5. Process telemetry data (data is already a list of points)
            # The original processing loop to ensure LapDist might still be useful if get_telemetry_points doesn't guarantee it
            # or if track_position needs conversion when track_length was only just found.
            valid_points = []
            for point in points_data: # Use points_data directly
                # Ensure all necessary keys are present from the get_telemetry_points fetch
                # LapDist might be directly available or needs to be calculated if only track_position was fetched and track_length is known
                ...
                elif 'LapDist' not in point: # If still no LapDist, this point might be problematic for x-axis
                    logger.debug(f"Point missing LapDist and cannot calculate: {point.get('timestamp')}")
                    continue
                
                # Ensure numeric types for graphable values, default to 0 if problematic after fetch
                ...
                point['LapDist'] = float(point['LapDist']) # Ensure LapDist is float
            ...
            if valid_points:
                ...
                is_incomplete, stopped_sections, log_msg = self._check_lap_completeness(valid_points, track_length, lap_id)
                ...
                logger.info(f"Displaying {len(valid_points)} raw telemetry points for lap {lap_id}")
                # Update both graphs with the same data
                # Wrap valid_points in a dictionary for update_graph, as expected by preprocess_telemetry_data
                ...
                logger.warning(f"No valid telemetry points after cleaning for lap {lap_id}")
                # Pass empty list for points in the dictionary when clearing
                empty_data_to_plot = {"points": []}
                if hasattr(self, 'throttle_graph'): self.throttle_graph.update_graph(empty_data_to_plot, track_length, track_context)
                if hasattr(self, 'steering_graph'): self.steering_graph.update_graph(empty_data_to_plot, track_length, track_context)
                # ADD THIS CALL FOR BRAKE GRAPH
                if hasattr(self, 'brake_graph'): self.brake_graph.update_graph(empty_data_to_plot, track_length, track_context) 
                # END ADDITION
                # Add call for speed graph
                if hasattr(self, 'speed_graph'): self.speed_graph.update_graph(empty_data_to_plot, track_length, track_context)
                if hasattr(self, 'graph_status_label'): self.graph_status_label.setVisible(False)

        except Exception as e:
            logger.exception(f"Error loading throttle data for lap {lap_id}: {e}")
            empty_data_to_plot = {"points": []}
            if hasattr(self, 'throttle_graph'): self.throttle_graph.update_graph(empty_data_to_plot, 0, {})
            if hasattr(self, 'steering_graph'): self.steering_graph.update_graph(empty_data_to_plot, 0, {})
            # ADD THIS CALL FOR BRAKE GRAPH
            if hasattr(self, 'brake_graph'): self.brake_graph.update_graph(empty_data_to_plot, 0, {}) 
            # END ADDITION
            # Add call for speed graph
            if hasattr(self, 'speed_graph'): self.speed_graph.update_graph(empty_data_to_plot, 0, {})
    ...
    def _fetch_track_context(self, client, track_id, track_length):
        ...
        try:
            ...
            # Markers
            marker_result = client.table("track_markers").select("track_position").eq("track_id", track_id).execute()
            if marker_result.data:
                markers = []
                for m in marker_result.data:
                    # Adjust to only get "track_position" and use an empty string for label
                    pos = m.get("track_position")
                    # Ensure lbl is always a string, even if just empty, to match original structure if downstream code expects it.
                    lbl = "" 
                    if isinstance(pos, (int, float)) and 0 <= pos <= 1:
                         markers.append((pos * track_length, lbl))
    ...
    def on_lap_selection_changed(self):
        ...
        # sender = self.sender() # No longer need sender
        # if not sender: return # No longer need sender
    ...
    def _on_new_session_created(self, session_uuid):
        """
        Handler for the new_trackpro_session_created signal.
        Refreshes the session list and selects the newly created session.
        """
        logger.info(f"New TrackPro session created notification received: {session_uuid}")
        
        # Add a slight delay to ensure the session is fully created in Supabase
        QTimer.singleShot(1000, lambda: self._refresh_and_select_new_session(session_uuid))
        
    def _refresh_and_select_new_session(self, session_uuid):
        """
        Refreshes the session list and selects the specified session.
        Called after a delay when a new session is created.
        """
        logger.info(f"Refreshing session list and selecting new session: {session_uuid}")
        
        # Remember current selections to try and restore if new one isn't immediately found
        current_session_id_before_refresh = self.session_combo.currentData()
        
        # Refresh the session list from the database
        self.refresh_session_list() # This populates self.session_combo
        
        # Find and select the new session in the combo box
        new_session_found_and_selected = False
        for i in range(self.session_combo.count()):
            if self.session_combo.itemData(i) == session_uuid:
                logger.info(f"Found new session UUID {session_uuid} at index {i}, selecting it.")
                self.session_combo.setCurrentIndex(i)
                # Crucially, when a brand new session is selected, we expect no laps yet.
                # So, clear lap lists and graphs instead of trying to load laps for it.
                self.left_lap_combo.clear()
                self.right_lap_combo.clear()
                self.left_lap_combo.addItem("No laps yet for this new session", None)
                self.right_lap_combo.addItem("No laps yet for this new session", None)
                self.clear_all_graphs() # Helper to clear all telemetry graphs
                new_session_found_and_selected = True
                break
                
        if not new_session_found_and_selected:
            logger.warning(f"New session UUID {session_uuid} not found in refreshed list after refresh_session_list(). Attempting to restore previous selection.")
            # Try to restore previous selection if new one not found or if refresh_session_list didn't immediately contain it
            if current_session_id_before_refresh:
                for i in range(self.session_combo.count()):
                    if self.session_combo.itemData(i) == current_session_id_before_refresh:
                        logger.info(f"Restoring previous session selection at index {i} ({current_session_id_before_refresh})")
                        self.session_combo.setCurrentIndex(i)
                        # Since we are restoring, assume laps should be loaded for this restored session
                        # This will trigger on_session_changed, which calls refresh_lap_list
                        break
            elif self.session_combo.count() > 0:
                logger.info("No previous session to restore, and new session not found. Selecting first available session.")
                self.session_combo.setCurrentIndex(0) # Select first available if nothing else
            else:
                logger.warning("Session combo is empty after refresh and new session not found.")
                self.left_lap_combo.clear()
                self.right_lap_combo.clear()
                self.left_lap_combo.addItem("No sessions available", None)
                self.right_lap_combo.addItem("No sessions available", None)
                self.clear_all_graphs()

    def clear_all_graphs(self):
        """Clears all telemetry graph widgets."""
        logger.debug("Clearing all telemetry graphs.")
        ...
        if hasattr(self, 'gear_graph') and self.gear_graph:
            self.gear_graph.clear_graph()
        if hasattr(self, 'rpm_graph') and self.rpm_graph:
            self.rpm_graph.clear_graph()
        # Add any other graphs that might exist
        if hasattr(self, 'combined_input_graph') and self.combined_input_graph:
            self.combined_input_graph.clear_graph()
        if hasattr(self, 'map_view_widget') and self.map_view_widget:
            self.map_view_widget.clear_map() # Assuming map has a clear method