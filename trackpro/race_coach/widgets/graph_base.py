...
class GraphBase(QWidget):
    ...
    def clear_graph(self):
        """Base method to clear graph data - subclasses should override and extend."""
        logger.info(f"Base clear_graph called for {self.__class__.__name__}")
        # Generic clearing - subclasses should clear their specific plot items (curves, etc.)
        if hasattr(self, 'plot_widget') and self.plot_widget:
            # Remove items added by GraphBase or that are common
            ...
            if hasattr(self, 'debug_info_text') and self.debug_info_text and hasattr(self.debug_info_text, 'hide'):
                self.debug_info_text.hide()
            if hasattr(self, 'clear_track_markers'): # If subclass has this specific method
                self.clear_track_markers()
            if hasattr(self, 'legend') and self.legend:
                self.legend.clear()
            # Call reset_view if it exists to reset zoom/pan
            if hasattr(self, 'reset_view'):
                self.reset_view()
        else:
            logger.warning(f"Cannot clear graph for {self.__class__.__name__}, plot_widget not found.")
