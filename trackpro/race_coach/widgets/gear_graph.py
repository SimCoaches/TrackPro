...
class GearGraphWidget(GraphBase):
    def __init__(self, parent=None):
        ...
        # Configure Y-Axis for gear (1-7+ gears) with better styling
        left_axis = self.plot_widget.getAxis('left')
        left_axis.setLabel("Gear", color='#e6edf3')
        ...
        title_item.setAttr('color', '#7dd3fc')
    ...
    def mouseMoved(self, pos):
        ...
        try:
            # Handle different position data formats robustly
            if isinstance(pos, tuple):
                from PyQt6.QtCore import QPointF
                if len(pos) >= 2:
                    pos = QPointF(pos[0], pos[1])
                elif len(pos) == 1:
                    # Sometimes we get a single-element tuple with QPointF inside
                    pos = pos[0]
                else:
            ...
            if self.plot_widget.sceneBoundingRect().contains(pos):
                ...
                # Find the actual gear values at the cursor position
                gear_value_a = None
                gear_value_b = None
                distance = x
                
                # Find the closest data point from Lap A
                if self.gear_curve.xData is not None and len(self.gear_curve.xData) > 0:
                    # Find the closest x point to the cursor
                    closest_idx = -1
                    min_distance = float('inf')
                    
                    for i, x_val in enumerate(self.gear_curve.xData):
                        dist = abs(x_val - x)
                    ...
                    if closest_idx >= 0:
                        # Get actual gear value from the data
                        if closest_idx < len(self.gear_curve.yData):
                            gear_value_a = int(self.gear_curve.yData[closest_idx])

                # In comparison mode, also get data from Lap B
                if self.comparison_mode and self.gear_curve_b.xData is not None and len(self.gear_curve_b.xData) > 0:
                    # Find the closest x point to the cursor
                    closest_idx = -1
                    min_distance = float('inf')
                    
                    for i, x_val in enumerate(self.gear_curve_b.xData):
                        dist = abs(x_val - x)
                        if dist < min_distance:
                            min_distance = dist
                            closest_idx = i
                    ...
                    if closest_idx >= 0:
                        # Get actual gear value from the data
                        if closest_idx < len(self.gear_curve_b.yData):
                            gear_value_b = int(self.gear_curve_b.yData[closest_idx])
                
                # Format gear display (handle special cases)
                def format_gear(gear_val):
                    if gear_val == -1:
                        return "R"
                    elif gear_val == 0:
                        return "N"
    ...
    def reset_view(self):
        ...
        try:
            # Clear existing data
            self.gear_curve.setData([], [])
            self.gear_curve_b.setData([], [])
            
            # Hide comparison curves
            self.gear_curve_b.hide()
            ...
            # Set Y range for gear (typically -1 to 7+ for reverse, neutral, and forward gears)
            self.plot_widget.setYRange(-1, 7, padding=0.1)
    ...
    def _detect_stopped_sections(self, distances, gears, min_points_stop=10, pos_tolerance=0.5):
        ...
        for i, dist in enumerate(distances):
            ...
            if bin_pos not in point_data:
                 ...
                 point_data[bin_pos] = {
                     'avg_gear': gears[i] if i < len(gears) else 0,
        ...
        for pos, count in pos_counts.items():
            if count >= min_points_stop:
                ...
                section_data = point_data.get(pos, {'avg_gear': 0})
    ...
    def add_distance_grid(self):
        ...
        grid_pen = pg.mkPen((80, 80, 80), width=1, style=Qt.PenStyle.DashLine)
    ...
    def update_graph(self, lap_data, track_length, track_context=None):
        ...
        try:
            logger.info(f"Updating gear graph with single lap data: {len(lap_data.get('points', [])) if lap_data else 0} points")
            
            # Validate lap data before processing
            is_valid, lap_data, debug_info = self.validate_telemetry_data(lap_data, 'gear')
            ...
            for item in list(self.plot_widget.items()):
                if item not in [self.gear_curve, self.gear_curve_b, self.vLine, self.hLine, self.label, self.start_finish_line, self.debug_info_text]:
            ...
            if not lap_data or not is_valid:
                self.gear_curve.setData([], []) # Clear main curves
                self.display_message("No Valid Gear Data")
                self.reset_view()
                return
            ...
            resampled_data = self.preprocess_telemetry_data(lap_data, track_length, ['Gear'])
            if not resampled_data:
                self.gear_curve.setData([], []) # Clear main curves
                self.display_message("Failed to preprocess gear data")
                self.reset_view()
                return
            ...
            self.gear_curve.setData(resampled_data['x_m'], resampled_data['Gear'])
            self.gear_curve.show()
            
            # Set legend
            if self.legend:
                self.legend.clear()
                self.legend.addItem(self.gear_curve, f"{self.label_a} Gear")
            
            # Set Y-axis range for gear (typically -1 to 7+ for reverse, neutral, and forward gears)
            gear_values = resampled_data['Gear']
            if len(gear_values) > 0:
                min_gear = min(gear_values)
                max_gear = max(gear_values)
                y_padding = 0.5
                self.plot_widget.setYRange(min_gear - y_padding, max_gear + y_padding, padding=0)
            else:
                self.plot_widget.setYRange(-1, 7, padding=0.1)
            ...
            if len(resampled_data['x_m']) > 0:
                ...
                logger.info(f"📊 [GEAR GRAPH] Setting X-axis range to actual data: {x_min:.1f} to {x_max:.1f}m (data range: {data_x_min:.1f} to {data_x_max:.1f}m)")
            ...
            for point in points:
                ...
                raw_gears.append(gear)
                
            # Detect and add stopped section markers
            stopped_sections = self._detect_stopped_sections(raw_distances, raw_gears)
            ...
            logger.error(f"Error updating gear graph: {e}", exc_info=True)
            self.gear_curve.setData([], [])
    ...
    def set_labels(self, label_a="Lap A", label_b="Lap B"):
        ...
        self.gear_curve.opts['name'] = f"{self.label_a} Gear"
        self.gear_curve_b.opts['name'] = f"{self.label_b} Gear"
    ...
    def update_graph_comparison(self, lap_a_data, lap_b_data, track_length, track_context=None, label_a=None, label_b=None):
        ...
        logger.info(f"Updating gear graph with comparison data for A:{len(lap_a_data['points']) if lap_a_data else 'None'} and B:{len(lap_b_data['points']) if lap_b_data else 'None'} points")
        ...
        is_valid_a, lap_a_data, debug_info_a = self.validate_telemetry_data(lap_a_data, 'gear')
        is_valid_b, lap_b_data, debug_info_b = self.validate_telemetry_data(lap_b_data, 'gear')
        ...
        global_y_min, global_y_max = -1, 7  # Default for gear
        
        # Process and plot data for Lap A if valid
        if is_valid_a:
            resampled_data_a = self.preprocess_telemetry_data(lap_a_data, track_length, ['Gear'])
            if resampled_data_a:
                # Update plot with resampled data
                self.gear_curve.setData(resampled_data_a['x_m'], resampled_data_a['Gear'])
                
                # Update global min/max values
                y_min_a = np.nanmin(resampled_data_a['Gear'])
                y_max_a = np.nanmax(resampled_data_a['Gear'])
                
                global_y_min = min(global_y_min, y_min_a)
                global_y_max = max(global_y_max, y_max_a)
                
                logger.info(f"Set Lap A gear data with {len(resampled_data_a['x_m'])} points")
            else:
                logger.warning("Failed to preprocess Lap A data")
                self.gear_curve.hide()
        else:
        ...
        if is_valid_b:
            resampled_data_b = self.preprocess_telemetry_data(lap_b_data, track_length, ['Gear'])
            if resampled_data_b:
                # Update plot with resampled data
                self.gear_curve_b.setData(resampled_data_b['x_m'], resampled_data_b['Gear'])
                
                # Update global min/max values
                y_min_b = np.nanmin(resampled_data_b['Gear'])
                y_max_b = np.nanmax(resampled_data_b['Gear'])
                
                global_y_min = min(global_y_min, y_min_b)
                global_y_max = max(global_y_max, y_max_b)
                
                logger.info(f"Set Lap B gear data with {len(resampled_data_b['x_m'])} points")
            else:
        ...
        if self.legend:
            ...
            if is_valid_a:
                self.legend.addItem(self.gear_curve, f"{self.label_a} Gear")
            if is_valid_b:
                self.legend.addItem(self.gear_curve_b, f"{self.label_b} Gear")
