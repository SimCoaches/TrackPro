...
class ThrottleGraphWidget(GraphBase):
    def __init__(self, parent=None):
        ...
        # Configure Y-Axis for throttle (0-100%) with fixed range
        left_axis = self.plot_widget.getAxis('left')
        left_axis.setLabel("Throttle (%)", color='#e6edf3')
        ...
        self.throttle_curve = self.plot_widget.plot(pen=smooth_pen_a, autoDownsample=False, clipToView=False, antialias=True)
        self.throttle_curve_b = self.plot_widget.plot(pen=smooth_pen_b, autoDownsample=False, clipToView=False, antialias=True)
        ...
        try:
            ...
            if self.plot_widget.sceneBoundingRect().contains(pos):
                ...
                if throttle_value_a is not None:
                    lap_a_data = {'throttle': f"{throttle_value_a:.0f}%"}
                    lap_b_data = {}
                    
                    if throttle_value_b is not None:
                        lap_b_data = {'throttle': f"{throttle_value_b:.0f}%"}
    ...
    def update_graph(self, lap_data, track_length, track_context=None):
        ...
        try:
            ...
            # Keep fixed Y-axis range for throttle (0% to 100%)
            ...
            if len(resampled_data['x_m']) > 0:
                ...
                logger.info(f"📊 [THROTTLE GRAPH] Setting X-axis range to actual data: {x_min:.1f} to {x_max:.1f}m (data range: {data_x_min:.1f} to {data_x_max:.1f}m)")
