...
try:
    ...
    from . import iracing_lap_saver as iracing_lap_saver_module # Import module for reloading
    ...
    def create_race_coach_widget(parent=None):
        ...
        try:
            ...
            try:
                ...
                # from .iracing_lap_saver import IRacingLapSaver # Original import, now handled by module import
                logger.info("RACE_COACH_INIT: About to reload iracing_lap_saver_module") # Debug print
                importlib.reload(iracing_lap_saver_module) # Reload the module to get the latest version
                iracing_lap_saver = iracing_lap_saver_module.IRacingLapSaver() # Instantiate from the reloaded module
                logger.info(f"RACE_COACH_INIT: Instantiated IRacingLapSaver. Has start_new_session_context: {hasattr(iracing_lap_saver, 'start_new_session_context')}") # Debug print
            ...
            try:
                ...
                # Store a reference to the API in the application's reference store
                try:
                    from .. import main
                    app_instance = None
                    # Find the app instance
                    for app in QApplication.instance().topLevelWidgets():
                        if hasattr(app, 'app') and hasattr(app.app, 'store_reference'):
                            app_instance = app.app
                            break
                    
                    if app_instance:
                        # Store reference to prevent garbage collection
                        iracing_api = app_instance.store_reference(iracing_api)
                        logger.info("Stored reference to SimpleIRacingAPI in application's reference store")
                    else:
                        logger.warning("Could not find application instance to store API reference")
                except Exception as e:
                    logger.warning(f"Could not store API reference in application: {e}")
                ...
                if iracing_lap_saver is not None:
                    ...
                    try:
                        ...
                        if user and hasattr(user, 'id') and user.is_authenticated:
                            ...
                            try:
                                logger.info("About to initialize iRacing session monitor thread...")
                                import threading
                                from .iracing_session_monitor import start_monitoring
                                # Pass the client object, user ID, lap saver, and API instance
                                monitor_thread = start_monitoring(supabase_client, user_id, iracing_lap_saver, iracing_api)
                                logger.info(f"iRacing session monitor thread started successfully: {monitor_thread is not None}")
