...
class CommunityMainWidget(QWidget):
    ...
    def create_modern_social_widget(self):
        ...
        if self.user_id and self.user_id != 'default_user':
            ...
            # Racing Activity Feed Tab - TrackPro-specific activities
            activity_widget = self.create_racing_activity_feed()
            tab_widget.addTab(activity_widget, "Racing Activity")
    ...
    def create_racing_activity_feed(self):
        ...
        # Header with Discord integration note
        header_layout = QVBoxLayout()
        
        title_label = QLabel("Racing Activity Feed")
        title_label.setFont(QFont(*CommunityTheme.FONTS['subheading']))
        title_label.setStyleSheet(f"color: {CommunityTheme.COLORS['text_primary']}; font-weight: bold;")
        
        discord_note = QLabel("💬 For chat and live communication, use the Discord tab →")
        ...
        # Share racing achievement section
        share_widget = QWidget()
        share_widget.setStyleSheet(f"""
            QWidget {{
                background-color: {CommunityTheme.COLORS['surface_darker']};
                border: 1px solid {CommunityTheme.COLORS['border']};
                border-radius: 6px;
        ...
        share_layout = QVBoxLayout(share_widget)
        share_layout.setSpacing(8)
        
        share_input = QTextEdit()
        share_input.setPlaceholderText("Share your latest lap time, setup breakthrough, or racing milestone...")
        share_input.setMaximumHeight(80)
        share_input.setStyleSheet(f"""
            QTextEdit {{
                background-color: {CommunityTheme.COLORS['surface']};
                border: 1px solid {CommunityTheme.COLORS['border']};
                border-radius: 4px;
        ...
        share_button = QPushButton("🏁 Share Racing Update")
        share_button.setStyleSheet(f"""
            QPushButton {{
                background-color: {CommunityTheme.COLORS['accent']};
                color: white;
                padding: 8px 16px;
        ...
        telemetry_button = QPushButton("📊 Share Telemetry")
        telemetry_button.setStyleSheet(f"""
            QPushButton {{
                background-color: {CommunityTheme.COLORS['active']};
                color: white;
                padding: 8px 16px;
        ...
        share_buttons_layout.addWidget(share_button)
        share_buttons_layout.addWidget(telemetry_button)
        share_buttons_layout.addStretch()
        
        share_layout.addWidget(share_input)
        share_layout.addLayout(share_buttons_layout)
        layout.addWidget(share_widget)
        
        # Racing activity feed
        scroll_area = QScrollArea()
        scroll_area.setWidgetResizable(True)
        scroll_area.setStyleSheet("QScrollArea { border: none; }")
        ...
        # Sample racing activities (these would come from database)
        racing_activities = [
            ("🏆", "Personal Best", "Set new personal best at Silverstone: 1:42.356", "2 hours ago"),
            ("📊", "Setup Shared", "Shared brake balance setup for Monza", "4 hours ago"),
            ("🎯", "Achievement", "Completed 'Consistency Master' challenge", "yesterday"),
            ("⚡", "Quick Race", "Finished P3 in daily quick race", "2 days ago"),
        ]
        
        for icon, activity_type, description, time_ago in racing_activities:
            activity_item = self.create_racing_activity_item(icon, activity_type, description, time_ago)
            feed_layout.addWidget(activity_item)
    ...
    def create_racing_friends_list(self):
        ...
        # Sample racing friends (these would come from database)
        racing_friends = [
            ("Alex_Racing", "🟢 In Race", "Silverstone: 1:43.234", "🏁"),
            ("SpeedDemon99", "🟡 Practicing", "Last seen: Monza", "📊"),
            ("TrackMaster", "🔴 Offline", "Best: Spa 2:18.456", "⚫"),
        ]
        
        for name, status, last_activity, status_icon in racing_friends:
            friend_item = self.create_racing_friend_item(name, status, last_activity, status_icon)
    ...
    def create_quick_chat_panel(self):
        ...
        # Sample quick notes
        sample_notes = [
            ("RacerMike", "Anyone up for Spa practice session?", "5 mins ago"),
            ("TrackQueen", "New setup file uploaded for McLaren", "12 mins ago"),
            ("You", "GG on that last race! Close finish", "25 mins ago"),
        ]
        
        for sender, note, time_ago in sample_notes:
            note_item = self.create_quick_note_item(sender, note, time_ago)
