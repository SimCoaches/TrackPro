...
class CalibrationWizard(QWizard):
    ...
    def __init__(self, hardware_input, parent=None):
        ...
        self.throttle_page = ThrottlePage(self.hardware_input)
        self.addPage(self.throttle_page)
        self.brake_page = BrakePage(self.hardware_input)
        self.addPage(self.brake_page)
        self.clutch_page = ClutchPage(self.hardware_input)
        self.addPage(self.clutch_page)
        ...
        self.cloud_status.setAlignment(Qt.AlignRight)
        if supabase.is_authenticated():
            self.cloud_status.setText("🔄 Cloud Sync Enabled")
            self.cloud_status.setStyleSheet("color: green;")
        else:
            self.cloud_status.setText("☁️ Sign in for Cloud Sync")
            self.cloud_status.setStyleSheet("color: gray;")
        self.setTitleBarWidget(self.cloud_status)
    ...
    def save_calibration(self):
        ...
        try:
            ...
            if hasattr(self.hardware_input, 'hardware'):
                ...
                for pedal in ['throttle', 'brake', 'clutch']:
                    if pedal in self.results:
                        ...
                        existing_ranges = self.hardware_input.hardware.axis_ranges.get(pedal, {})
                        min_deadzone = existing_ranges.get('min_deadzone', 0)
                        max_deadzone = existing_ranges.get('max_deadzone', 0)
                        
                        self.hardware_input.hardware.axis_ranges[pedal] = {
                ...
                self.hardware_input.hardware.save_axis_ranges()
                
                # Save axis mappings if they were changed
                for pedal in ['throttle', 'brake', 'clutch']:
                    if pedal in self.results and 'axis' in self.results[pedal]:
                        self.hardware_input.hardware.update_axis_mapping(pedal, self.results[pedal]['axis'])
                
                # Also save the calibration curve data
                calibration_data = self.hardware_input.hardware.calibration
                ...
                self.hardware_input.hardware.save_calibration(calibration_data)
                ...
                self.calibration_results = {
                    'throttle': self.results['throttle'],
                    'brake': self.results['brake'],
                    'clutch': self.results['clutch']
                }
    ...
    def show_all_axes(self, pedal_name):
        ...
        if not hasattr(self.hardware_input, 'hardware') or not self.hardware_input.hardware:
            return
        
        try:
            # Get joystick
            joystick = self.hardware_input.hardware.joystick
    ...
    def update_inputs(self):
        """Update all input displays with current pedal values."""
        
        try:
            # Read all axes directly - use direct joystick access to avoid pygame overhead
            if self.hardware_input.hardware and self.hardware_input.hardware.pedals_connected and self.hardware_input.hardware.joystick:
                # Only pump events when joystick is connected
                pygame.event.pump()
                joystick = self.hardware_input.hardware.joystick
