...
class HardwareInput:
    ...
    def _sync_with_cloud(self):
        """Sync calibration data with cloud storage."""
        try:
            if supabase.is_authenticated():
                user = supabase.get_user()
                if user:
                    # Get cloud calibrations
                    cloud_calibrations = calibration_manager.get_user_calibrations(user.id)
                    
                    # For each pedal, sync local and cloud calibrations
                    for pedal in ['throttle', 'brake', 'clutch']:
                        # Find cloud calibration for this pedal
                        cloud_cal = next((cal for cal in cloud_calibrations 
                                       if cal['name'] == f"{pedal}_default"), None)
                        
                        if cloud_cal:
                            # If cloud calibration exists, update local
                            self.calibration[pedal] = cloud_cal['data']
                            logger.info(f"Loaded cloud calibration for {pedal}")
                        else:
                            # If no cloud calibration, save local to cloud
                            calibration_manager.save_calibration(
                                user_id=user.id,
                                name=f"{pedal}_default",
                                data=self.calibration[pedal]
                            )
                            logger.info(f"Saved local calibration to cloud for {pedal}")
