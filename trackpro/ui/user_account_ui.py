...
from PyQt6.QtWidgets import (
    ...
    QFrame, QSizePolicy, QMessageBox, QDialog, QTabWidget, QStackedWidget,
    QGridLayout, QListWidget, QListWidgetItem, QAbstractItemView, QFileDialog
)
...
class UserAccountMainWidget(QWidget):
    ...
    def load_2fa_settings(self):
        """Load current 2FA settings from the user profile."""
        try:
            user_data = enhanced_user_manager.get_complete_user_profile(self.current_user_id)
            if user_data:
                # Get 2FA fields
                phone_number = user_data.get('phone_number', '')
                twilio_verified = user_data.get('twilio_verified', False)
                ...
                if twilio_verified:
                    # Phone is verified, enable 2FA toggle
                    self.twofa_checkbox.setEnabled(True)
                    self.twofa_checkbox.setChecked(is_2fa_enabled)
                    
                    # Pre-fill phone but allow editing for re-verification
                    self.send_code_btn.setText("Re-verify Number")
                
        except Exception as e:
            logger.error(f"Error loading 2FA settings: {e}")
    
    def on_phone_number_changed(self):
        """Handle phone number field changes."""
        # If user changes phone number, they need to re-verify
        try:
            user_data = enhanced_user_manager.get_complete_user_profile(self.current_user_id)
            if user_data:
                ...
                if stored_phone and current_phone != stored_phone:
                    # Phone number changed, disable 2FA and require re-verification
                    self.twofa_checkbox.setEnabled(False)
                    self.twofa_checkbox.setChecked(False)
                    self.send_code_btn.setText("Send Code")
                    
                    # Update database to mark as unverified
                    enhanced_user_manager.update_user_profile(self.current_user_id, {
                        'twilio_verified': False,
                        'is_2fa_enabled': False
                    })
                    
        except Exception as e:
            logger.error(f"Error handling phone number change: {e}")
    ...
    def send_verification_code(self):
        ...
        if not TWILIO_AVAILABLE or not twilio_service:
            QMessageBox.critical(self, "Error", "SMS 2FA is not configured. Please contact support.")
            return
        ...
        try:
            ...
            if result['success']:
                # Save phone number to database (unverified)
                enhanced_user_manager.update_user_profile(self.current_user_id, {
                ...
                QMessageBox.information(self, "SMS Sent", 
                    f"A verification code was sent to {phone_number}. Please enter the 6-digit code below.")
            else:
                QMessageBox.warning(self, "Error", f"Failed to send verification code: {result['message']}")
                
        except Exception as e:
            logger.error(f"Error sending verification code: {e}")
            QMessageBox.critical(self, "Error", f"Failed to send code: {str(e)}")
    ...
    def verify_code(self):
        ...
        try:
            ...
            if result['success']:
                ...
                QMessageBox.information(self, "Verified", 
                    "Phone number verified successfully! You can now enable Two-Factor Authentication.")
    ...
    def on_2fa_toggled(self):
        """Handle 2FA toggle state changes."""
        try:
            is_enabled = self.twofa_checkbox.isChecked()
            
            if is_enabled:
                # Enabling 2FA
                reply = QMessageBox.question(self, "Enable 2FA",
                    "Are you sure you want to enable Two-Factor Authentication? "
                    "You will be required to enter a verification code at login.",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    ...
                    QMessageBox.information(self, "2FA Enabled", 
                        "Two-factor authentication is now enabled. You will be required to enter a verification code at login.")
                else:
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    ...
                    # User cancelled, check the box again
                    self.twofa_checkbox.setChecked(True)
                    
        except Exception as e:
            logger.error(f"Error toggling 2FA: {e}")
            QMessageBox.critical(self, "Error", "An error occurred while updating 2FA settings.")
