...
class MainWindow(QMainWindow):
    ...
    def open_account_settings(self):
        ...
        try:
            ...
            # For now, open the Community interface and navigate to Account section
            # This is a simpler approach that avoids complex imports
            self.open_community_interface()
            
            # Try to find the community widget and switch to account tab
            try:
                for i in range(self.stacked_widget.count()):
                    widget = self.stacked_widget.widget(i)
                    if hasattr(widget, 'switch_to_account_section'):
                        widget.switch_to_account_section()
                        break
                    elif hasattr(widget, 'stacked_widget') and hasattr(widget.stacked_widget, 'setCurrentIndex'):
                        # Try to switch to account section (usually the last tab)
                        widget.stacked_widget.setCurrentIndex(widget.stacked_widget.count() - 1)
                        break
            except Exception as nav_error:
                logger.debug(f"Could not auto-navigate to account section: {nav_error}")
