...
class SpeedGraphWidget(GraphBase):
    ...
    def update_graph_comparison(self, lap_data_a, lap_data_b, track_length=0, metadata=None):
        ...
        logger.info(f"Updating speed graph with comparison data. Track length: {track_length}m")
        ...
        # Validate data
        valid_a, cleaned_a, debug_a = self.validate_telemetry_data(lap_data_a, 'speed')
        valid_b, cleaned_b, debug_b = self.validate_telemetry_data(lap_data_b, 'speed')
        
        # Log the validation results
        logger.info(f"Lap A speed data validation: {valid_a} ({debug_a['valid_points']}/{debug_a['original_points']} points valid)")
        logger.info(f"Lap B speed data validation: {valid_b} ({debug_b['valid_points']}/{debug_b['original_points']} points valid)")
        
        # Check if we have valid data for either lap
        if not valid_a and not valid_b:
            logger.error("No valid speed data for either lap")
            self.display_message("No valid speed data available for comparison")
            return
        ...
        self.plot_widget.setTitle("Speed Comparison")
        
        # Resample the data for Lap A
        if valid_a:
            resampled_a = self.preprocess_telemetry_data(
                cleaned_a, track_length, ['Speed'], resolution=self.distance_resolution
            )
        else:
            resampled_a = None
        ...
        if valid_b:
            resampled_b = self.preprocess_telemetry_data(
                cleaned_b, track_length, ['Speed'], resolution=self.distance_resolution
            )
        else:
            resampled_b = None
        ...
        if resampled_a:
            ...
            self.speed_curve.setVisible(True)
        else:
        ...
        if resampled_b:
            ...
            self.speed_curve_b.setVisible(True)
        else:
        ...
        logger.info("Speed comparison graph updated successfully")
