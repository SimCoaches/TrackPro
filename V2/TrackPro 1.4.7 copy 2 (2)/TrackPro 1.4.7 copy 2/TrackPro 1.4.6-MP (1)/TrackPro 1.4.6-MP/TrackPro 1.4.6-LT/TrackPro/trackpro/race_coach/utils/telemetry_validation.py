...
def calculate_coverage_percentage(telemetry_points):
    ...
    positions = [p.get('track_position', p.get('LapDistPct', 0)) for p in telemetry_points 
                if (p.get('track_position') is not None or p.get('LapDistPct') is not None)
                and isinstance(p.get('track_position', p.get('LapDistPct', 0)), (int, float))
                and 0 <= p.get('track_position', p.get('LapDistPct', 0)) <= 1]
    
    if not positions:
        return 0.0
    
    # Use a different approach to calculate coverage by tracking which segments are visited
    segments = [0] * 100  # Divide track into 100 segments
    for pos in positions:
        segment = min(99, max(0, int(pos * 100)))  # Convert position to segment index
        segments[segment] = 1  # Mark segment as visited
    
    # Calculate coverage based on number of visited segments
    coverage = (sum(segments) / 100.0) * 100.0
