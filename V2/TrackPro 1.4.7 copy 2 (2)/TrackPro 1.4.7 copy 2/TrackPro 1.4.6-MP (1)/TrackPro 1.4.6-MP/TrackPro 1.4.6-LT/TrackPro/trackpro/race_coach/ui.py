...
from .threaded_data_retriever import ThreadedDataRetriever  # Import our threaded data retriever

# Import our threaded data retriever
from .threaded_data_retriever import ThreadedDataRetriever
...
class TelemetryComparisonWidget(QWidget):
    ...
    def get_track_length(self):
        """Safely retrieves the current track length."""
        try:
            # Attempt to get track_length from the monitor's state
            # This is the most reliable source when live telemetry is active
            try:
                if hasattr(self, 'ir_monitor') and self.ir_monitor and hasattr(self.ir_monitor, 'current_session_info') and self.ir_monitor.current_session_info:
                    track_length = self.ir_monitor.current_session_info.get('track_length', 0)
                    if track_length > 0:
                        # logger.debug(f"get_track_length from ir_monitor: {track_length}")
                        return float(track_length)
            except AttributeError:
                # Continue to other methods if ir_monitor doesn't exist
                pass

            # Try to get track_length from the session_info dictionary (updated via signal)
            if hasattr(self, 'session_info') and self.session_info:
                track_length = self.session_info.get('track_length', 0)
                if track_length > 0:
                    logger.debug(f"get_track_length from session_info: {track_length}")
                    return float(track_length)

            # Try to get track_length from iracing_api directly
            if hasattr(self, 'iracing_api') and self.iracing_api:
                # Check if SimpleIRacingAPI state is accessible
                if hasattr(self.iracing_api, 'state'):
                    # Access track_length attribute directly (State object doesn't have get method)
                    if hasattr(self.iracing_api.state, 'track_length'):
                        track_length = self.iracing_api.state.track_length
                        if track_length > 0:
                            logger.debug(f"get_track_length from iracing_api.state: {track_length}")
                            return float(track_length)
                
                # Try to access ir_sdk.state directly if SimpleIRacingAPI exposes it
                if hasattr(self.iracing_api, 'ir_sdk') and hasattr(self.iracing_api.ir_sdk, 'state'):
                    # Access track_length attribute directly (State object doesn't have get method)
                    if hasattr(self.iracing_api.ir_sdk.state, 'track_length'):
                        track_length = self.iracing_api.ir_sdk.state.track_length
                        if track_length > 0:
                            logger.debug(f"get_track_length from iracing_api.ir_sdk.state: {track_length}")
                            return float(track_length)

            # Fallback: Try to get it from the selected session's data if available
            # This is useful when reviewing past sessions where live telemetry isn't running
            selected_session_id = self.session_combo.currentData()
            if selected_session_id and hasattr(self, 'sessions_data') and self.sessions_data:
                session_details = next((s for s in self.sessions_data if s['id'] == selected_session_id), None)
                if session_details and 'track_length' in session_details and session_details['track_length'] is not None:
                    track_length = session_details['track_length']
                    if track_length > 0:
                        logger.debug(f"get_track_length from session_details: {track_length}")
                        return float(track_length)
            
            # Last resort - use a hardcoded default based on the most recent track seen in logs (if available)
            # This at least gives us a value that can be used rather than failing completely
            logger.warning("Track length could not be determined from any source, using default value of 550.0 meters.")
            return 550.0  # Default based on recent session data observed in the logs
        except Exception as e:
            logger.error(f"Error getting track length: {e}", exc_info=True)
            return 550.0  # Default fallback value if all methods fail
...
class RaceCoachWidget(QWidget):
    ...
    def __init__(self, parent=None, iracing_api=None):
        ...
        # Initialize threaded data retriever
        self.data_retriever = ThreadedDataRetriever()
        
        # Initialize UI
        self.setup_ui()
        
        # Connect data retriever signals
        self.data_retriever.telemetry_fetched.connect(self._on_telemetry_fetched)
        self.data_retriever.laps_fetched.connect(self._on_laps_fetched)
        self.data_retriever.sessions_fetched.connect(self._on_sessions_fetched)
        self.data_retriever.error.connect(self._on_data_fetch_error)
    ...
    def refresh_session_list(self):
        ...
        self.data_retriever.fetch_sessions(limit=100, user_only=True)

    def refresh_lap_list(self, session_id):
        if not session_id:
            self.left_lap_combo.clear()
            self.right_lap_combo.clear()
            # logger.info("Session ID is None, cleared lap lists.")
            return
        ...
        # Use the threaded data retriever
        self.data_retriever.fetch_laps(session_id)
        # The actual population of the comboboxes will happen in _on_laps_fetched
        # Removed problematic logging line that caused NameError

    def on_compare_clicked(self):
        lap_a_id = self.left_lap_combo.currentData()
        lap_b_id = self.right_lap_combo.currentData()

        if not (lap_a_id and lap_b_id and lap_a_id != "ALL_LAPS" and lap_b_id != "ALL_LAPS" and lap_a_id != lap_b_id):
            logger.info("Comparison button clicked with insufficient or invalid lap selection.")
            if hasattr(self, 'loading_comparison_data_label'): # Ensure this attribute exists
                 self.loading_comparison_data_label.setText("Select two different valid laps to compare.")
                 self.loading_comparison_data_label.setVisible(True)
            else: # Fallback if label not set up
                 QMessageBox.information(self, "Lap Selection", "Please select two different valid laps for comparison.")
            
            # Attempt to display single lap A if it's valid
            if lap_a_id and lap_a_id != "ALL_LAPS":
                # Instead of directly calling on_lap_selection_changed, which might re-trigger comparison,
                # call the single lap loading logic directly or ensure on_lap_selection_changed handles this case.
                # For now, let's ensure single lap A is loaded if possible.
                self.load_telemetry_for_lap_a(lap_a_id) 
            else:
                self.clear_all_graphs() # Clear if Lap A is also not valid for single display
                if hasattr(self, 'loading_comparison_data_label'): self.loading_comparison_data_label.setVisible(False)
            return

        logger.info(f"Compare button clicked. Starting telemetry fetch for Lap A ({lap_a_id}) and Lap B ({lap_b_id}) using TelemetryFetchWorker.")
        
        if hasattr(self, 'loading_comparison_data_label'): 
            self.loading_comparison_data_label.setText("Loading comparison data...")
            self.loading_comparison_data_label.setVisible(True)
        else:
            logger.warning("'loading_comparison_data_label' not found in UI for status updates.")

        QApplication.setOverrideCursor(Qt.WaitCursor)
        self.compare_button.setEnabled(False)
        self.compare_button.setText("Loading...")

        # Ensure previous worker/thread is cleaned up if any
        if hasattr(self, 'telemetry_fetch_thread') and self.telemetry_fetch_thread and self.telemetry_fetch_thread.isRunning():
            logger.warning("Previous telemetry fetch thread still running. Attempting to stop it.")
            if hasattr(self, 'telemetry_fetch_worker') and self.telemetry_fetch_worker:
                self.telemetry_fetch_worker.cancel() # Assuming worker has a cancel method
            self.telemetry_fetch_thread.quit()
            if not self.telemetry_fetch_thread.wait(2000): # Wait for 2s
                logger.warning("Telemetry fetch thread did not quit gracefully, terminating.")
                self.telemetry_fetch_thread.terminate()
                self.telemetry_fetch_thread.wait() # Wait for termination
            self.telemetry_fetch_thread = None
            self.telemetry_fetch_worker = None

        # Create and start the TelemetryFetchWorker
        self.telemetry_fetch_worker = TelemetryFetchWorker(lap_a_id, lap_b_id)
        ...
        self.telemetry_fetch_thread.finished.connect(self.telemetry_fetch_worker.deleteLater)
        self.telemetry_fetch_thread.finished.connect(self._cleanup_telemetry_fetch_references) 

        self.telemetry_fetch_thread.start()
        logger.info("TelemetryFetchWorker thread started for comparison.")

    def load_telemetry_for_lap_a(self, lap_id):
        if not lap_id:
            logger.warning("Cannot load telemetry data: No lap ID provided")
            return
            
        logger.info(f"Requesting telemetry data fetch for lap: {lap_id}")
        
        # Show loading indicator in status label if it exists
        if hasattr(self, 'graph_status_label'):
            self.graph_status_label.setText("Loading telemetry data...")
            self.graph_status_label.setStyleSheet("color: #0066CC; font-weight: bold;")
            self.graph_status_label.setVisible(True)
        
        # Define required columns for telemetry fetch
        required_columns = ["timestamp", "throttle", "brake", "track_position", "steering", "speed"]
        
        # Get track length with additional context checking to prevent AttributeError
        track_length = self.get_track_length()
        
        if track_length <= 0:
            logger.warning(f"Invalid track_length ({track_length}). Attempting fallback...")
            # Try to get track length from session if available
            session_id = self.session_combo.currentData() if hasattr(self, 'session_combo') else None
            if session_id and hasattr(self, 'sessions_data') and self.sessions_data:
                session_details = next((s for s in self.sessions_data if s['id'] == session_id), None)
                if session_details and 'track_length' in session_details and session_details['track_length'] is not None:
                    try:
                        track_length_from_session = float(session_details['track_length'])
                        if track_length_from_session > 0:
                            track_length = track_length_from_session
                            logger.info(f"Using track_length from session data: {track_length}")
                    except (ValueError, TypeError) as e:
                        logger.error(f"Error converting track_length from session data: {e}")
            
            # Use default if still invalid
            if track_length <= 0:
                track_length = 1000.0  # Use 1km as a consistent default
                logger.info(f"Using default track_length: {track_length}")
                
        # Queue the fetch request with validated track_length
        self.data_retriever.fetch_telemetry(lap_id, columns=required_columns, track_length=track_length)
    ...
    def _on_telemetry_fetch_finished(self, left_data, right_data):
        ...
        # left_data and right_data are expected to be dicts like {'stats': ..., 'points': [...]} or None
        success_left = left_data is not None and isinstance(left_data.get('points'), list) and len(left_data['points']) > 0
        success_right = right_data is not None and isinstance(right_data.get('points'), list) and len(right_data['points']) > 0

        logger.info(f"Data for comparison - Left success: {success_left}, Right success: {success_right}")
        if left_data:
            logger.info(f"Left data initial points: {len(left_data.get('points', [])) if left_data.get('points') is not None else 'No points key or None'}")
        else:
            logger.info("Left data is None initially.")
        if right_data:
            logger.info(f"Right data initial points: {len(right_data.get('points', [])) if right_data.get('points') is not None else 'No points key or None'}")
        else:
            logger.info("Right data is None initially.")
        ...
        if not success_left and not success_right:
            ...
            if hasattr(self, 'loading_comparison_data_label'):
                self.loading_comparison_data_label.setText("Error: Could not load data for comparison.")
                self.loading_comparison_data_label.setVisible(True) # Keep visible with error
            return

        # --- SupaBase Format Data Processing - Convert track_position to LapDist  ---
        def ensure_lap_dist_field(lap_data_dict): # Renamed arg for clarity
            if not lap_data_dict or not lap_data_dict.get('points'): return lap_data_dict
            
            track_length = lap_data_dict.get('track_length', 0)
            if track_length <= 0:
                track_length = self.get_track_length() 
                lap_data_dict['track_length'] = track_length
            
            # Make sure LapDist exists for each point
            for point in lap_data_dict['points']:
                if 'LapDist' not in point and 'track_position' in point:
                    point['LapDist'] = point['track_position'] * track_length if track_length > 0 else point['track_position'] * 1000.0
        ...
        def preprocess_lap_data(lap_data_dict): # Renamed arg
            if not lap_data_dict or not lap_data_dict.get('points'):
                logger.warning("preprocess_lap_data: called with None or no points.")
                return None # Return None if essential data is missing

            valid_points = []
            raw_point_count = len(lap_data_dict.get('points', []))

            for point in lap_data_dict.get('points', []):
                if 'LapDist' not in point:
                    continue

                throttle_val = point.get('throttle', point.get('Throttle'))
                brake_val = point.get('brake', point.get('Brake'))
                steering_val = point.get('steering', point.get('SteeringWheelAngle'))
                speed_val = point.get('speed', point.get('Speed'))

                if any(val is None for val in [throttle_val, brake_val, steering_val, speed_val]):
                    continue
                
                try:
                    valid_points.append({
                        'LapDist': float(point['LapDist']),
                        'throttle': float(throttle_val),
                        'brake': float(brake_val),
                        'steering': float(steering_val),
            ...
            # Return a new dictionary or modify a copy
            # To avoid modifying the original dict if it's used elsewhere, create a new one
            processed_data = lap_data_dict.copy() # shallow copy is fine if points list is replaced
            processed_data['points'] = valid_points
            logger.info(f"Preprocessed lap data: {len(valid_points)} valid points from {raw_point_count} raw points. Original success status: {lap_data_dict is not None and lap_data_dict.get('points')}")
            
            # If after processing there are no points, this lap should be considered unsuccessful for display
            if not valid_points:
                 logger.warning(f"Preprocessing resulted in 0 valid points from {raw_point_count} raw points.")
                 return None # Indicates failure after processing for this lap
            return processed_data
        
        processed_left_data = None
        processed_right_data = None

        if success_left:
            logger.info(f"Preprocessing Lap A (left_data initially has {len(left_data.get('points',[]))} points)")
            processed_left_data = preprocess_lap_data(left_data)
        if success_right:
            logger.info(f"Preprocessing Lap B (right_data initially has {len(right_data.get('points',[]))} points)")
            processed_right_data = preprocess_lap_data(right_data)

        # Update success flags based on processed data
        final_success_left = processed_left_data is not None and processed_left_data.get('points')
        final_success_right = processed_right_data is not None and processed_right_data.get('points')

        logger.info(f"After preprocessing - Left success: {final_success_left}, Right success: {final_success_right}")
        if final_success_left:
            logger.info(f"Processed Left data has {len(processed_left_data['points'])} points.")
        if final_success_right:
            logger.info(f"Processed Right data has {len(processed_right_data['points'])} points.")


        # --- Update UI with fetched data --- #
        current_track_length = self.get_track_length()

        if final_success_left and final_success_right:
            logger.info("Both laps have data after processing, proceeding with comparison display.")
            self.throttle_graph.update_graph_comparison(processed_left_data, processed_right_data, current_track_length)
            self.brake_graph.update_graph_comparison(processed_left_data, processed_right_data, current_track_length)
            self.steering_graph.update_graph_comparison(processed_left_data, processed_right_data, current_track_length)
            self.speed_graph.update_graph_comparison(processed_left_data, processed_right_data, current_track_length)
            if hasattr(self, 'loading_comparison_data_label'):
                self.loading_comparison_data_label.setVisible(False)
            logger.info("Lap comparison graphs updated successfully.")
        elif final_success_left:
            logger.warning("Only Lap A has valid data after processing. Displaying single lap view for Lap A.")
            self.throttle_graph.update_graph_single(processed_left_data, current_track_length)
            self.brake_graph.update_graph_single(processed_left_data, current_track_length)
            self.steering_graph.update_graph_single(processed_left_data, current_track_length)
            self.speed_graph.update_graph_single(processed_left_data, current_track_length)
            if hasattr(self, 'loading_comparison_data_label'):
                self.loading_comparison_data_label.setText("Comparison incomplete: Lap B data missing or invalid. Showing Lap A.")
                self.loading_comparison_data_label.setVisible(True)
        elif final_success_right:
            logger.warning("Only Lap B has valid data after processing. Displaying single lap view for Lap B.")
            self.throttle_graph.update_graph_single(processed_right_data, current_track_length)
            self.brake_graph.update_graph_single(processed_right_data, current_track_length)
            self.steering_graph.update_graph_single(processed_right_data, current_track_length)
            self.speed_graph.update_graph_single(processed_right_data, current_track_length)
            if hasattr(self, 'loading_comparison_data_label'):
                self.loading_comparison_data_label.setText("Comparison incomplete: Lap A data missing or invalid. Showing Lap B.")
                self.loading_comparison_data_label.setVisible(True)
        else:
            logger.error("Neither lap has sufficient data for display after processing.")
            self.throttle_graph.clear_graph()
            self.brake_graph.clear_graph()
            self.steering_graph.clear_graph()
            self.speed_graph.clear_graph()
            if hasattr(self, 'loading_comparison_data_label'):
                self.loading_comparison_data_label.setText("Error: No valid data for comparison display.")
                self.loading_comparison_data_label.setVisible(True)

        # --- Additional debug info --- #
        self._debug_check_curve_state()
        logger.info("Lap comparison processing in _on_telemetry_fetch_finished completed.")
    ...
    def _on_telemetry_fetch_error(self, msg_left, msg_right):
        ...
        # Basic UI restoration (full happens in cleanup)
        self.compare_button.setText("Compare Laps")
    ...
    def load_throttle_data(self, lap_id: str):
        """Load throttle data for a specific lap and update the throttle graph."""
        if not lap_id:
    ...
    def on_lap_selection_changed(self):
        ...
        if can_compare:
            logger.info(f"Lap selection changed. Valid for comparison. Triggering on_compare_clicked for Laps {left_lap_id} and {right_lap_id}")
            # No need to set loading_comparison_data_label here, on_compare_clicked will do it.
            ...
            if hasattr(self, 'loading_comparison_data_label'): 
                self.loading_comparison_data_label.setVisible(False) # Hide comparison loading label
            
            # Clear comparison elements in graphs explicitly
            self._set_graphs_to_single_mode() # New helper method

            if left_lap_id and left_lap_id != "ALL_LAPS":
                logger.info(f"Updating single lap view for Lap ID: {left_lap_id}")
                self.load_telemetry_for_lap_a(left_lap_id) 
            else:
    ...
    def _set_graphs_to_single_mode(self):
        """Helper to reset graphs to single lap display mode."""
        graphs = [self.throttle_graph, self.brake_graph, self.steering_graph, self.speed_graph]
        for graph in graphs:
            if graph is None: continue # Skip if a graph isn't initialized
            if hasattr(graph, 'comparison_mode'): # Check if graph object supports this
                graph.comparison_mode = False
            # Assuming plot_b_name is an attribute on the graph instance holding the name of the B curve
            if hasattr(graph, 'plot_b_name') and hasattr(graph, getattr(graph, 'plot_b_name')):
                curve_b = getattr(graph, getattr(graph, 'plot_b_name'))
                if curve_b: # Check if the curve object itself exists
                    curve_b.setData([], []) # Clear data for B curve
                    curve_b.setVisible(False) # Hide B curve
            elif hasattr(graph, 'throttle_curve_b'): # Fallback for specific known name (example)
                if hasattr(graph.throttle_curve_b, 'setData'): graph.throttle_curve_b.setData([],[])
                if hasattr(graph.throttle_curve_b, 'setVisible'): graph.throttle_curve_b.setVisible(False)
            # Add similar fallbacks for brake_curve_b, steering_curve_b, speed_curve_b if needed
            
            if hasattr(graph, 'plot_widget') and hasattr(graph.plot_widget, 'setTitle'):
                 # Construct title carefully, avoid error if objectName is not as expected
                 try:
                     base_title = graph.objectName().replace("_graph", "").replace("GraphWidget","").capitalize()
                     original_title = f"{base_title} vs Distance" if base_title else "Input vs Distance"
                     graph.plot_widget.setTitle(original_title)
                 except Exception as e:
                     logger.warning(f"Could not reset title for graph {graph.objectName()}: {e}")
                     graph.plot_widget.setTitle("Input vs Distance") # Generic fallback title
            if hasattr(graph, 'clear_comparison_specific_elements'): # For any other comparison elements
                graph.clear_comparison_specific_elements()
        logger.debug("Graphs set to single lap display mode.")

    def clear_all_graphs(self):
        """Clears all telemetry graphs and resets them to single display mode."""
        logger.info("Clearing all telemetry graphs.")
        self._set_graphs_to_single_mode() # Ensure comparison artifacts are cleared
        
        graphs_to_clear = [self.throttle_graph, self.brake_graph, self.steering_graph, self.speed_graph]
        for graph in graphs_to_clear:
            if graph and hasattr(graph, 'update_graph_single'):
                graph.update_graph_single(None, 0) # Clears the primary curve as well
            elif graph and hasattr(graph, 'clear_graph'): # Alternative clear method
                graph.clear_graph()
    ...
    def refresh_supabase_auth(self):
        ...
        return True
            ...
            # Replace existing on_compare_clicked method with this:
    def on_compare_clicked(self):
        lap_a_id = self.left_lap_combo.currentData()
        lap_b_id = self.right_lap_combo.currentData()
        ...
        if hasattr(self, 'telemetry_fetch_thread') and self.telemetry_fetch_thread and self.telemetry_fetch_thread.isRunning():
            ...
            self.telemetry_fetch_thread.quit()
            if not self.telemetry_fetch_thread.wait(2000): 
                logger.warning("Telemetry fetch thread did not quit gracefully, terminating.")
                self.telemetry_fetch_thread.terminate()
                self.telemetry_fetch_thread.wait() 
            self.telemetry_fetch_thread = None
            self.telemetry_fetch_worker = None
    ...
    # Replace existing on_lap_selection_changed method with this:
    def on_lap_selection_changed(self):
        """Handle lap selection changes from either combo box."""
        ...
        if can_compare:
            ...
            self._set_graphs_to_single_mode() 
    ...
    # Add these new helper methods to the RaceCoachWidget class:
    def _set_graphs_to_single_mode(self):
        """Helper to reset graphs to single lap display mode."""
        graphs = [self.throttle_graph, self.brake_graph, self.steering_graph, self.speed_graph]
        for graph in graphs:
            if graph is None: continue 
            if hasattr(graph, 'comparison_mode'): 
                graph.comparison_mode = False
            
            # Generalized way to find and clear the 'B' curve
            # Common convention might be plot_item_b or curve_b or a specific name like throttle_curve_b
            possible_b_curve_names = [
                getattr(graph, 'plot_b_name', None), # If graph explicitly defines its B curve name
                'throttle_curve_b', 'brake_curve_b', 'steering_curve_b', 'speed_curve_b', # Common specific names
                'plot_b', 'curve_b' # Generic common names
            ]
            
            cleared_b_curve = False
            for b_curve_name in possible_b_curve_names:
                if b_curve_name and hasattr(graph, b_curve_name):
                    curve_b_attr = getattr(graph, b_curve_name, None)
                    if curve_b_attr and hasattr(curve_b_attr, 'setData') and hasattr(curve_b_attr, 'setVisible'):
                        curve_b_attr.setData([], []) 
                        curve_b_attr.setVisible(False)
                        cleared_b_curve = True
                        break # Found and cleared the B curve
            if not cleared_b_curve:
                logger.debug(f"Could not find or clear a B curve for graph: {graph.objectName()}")
            ...
            if hasattr(graph, 'plot_widget') and hasattr(graph.plot_widget, 'setTitle'):
                 try:
                     ...
                     logger.warning(f"Could not reset title for graph {graph.objectName()}: {e}")
                     graph.plot_widget.setTitle("Input vs Distance") 
            if hasattr(graph, 'clear_comparison_specific_elements'): 
                graph.clear_comparison_specific_elements()
        logger.debug("Graphs set to single lap display mode.")
    ...
    def clear_all_graphs(self):
        ...
        for graph in graphs_to_clear:
            if graph and hasattr(graph, 'update_graph_single'): 
                graph.update_graph_single(None, 0) 
            elif graph and hasattr(graph, 'clear_graph'): 
                graph.clear_graph()
    ...
    def get_track_length(self):
        ...
        try:
            ...
            if hasattr(self, 'iracing_api') and self.iracing_api:
                # Check if SimpleIRacingAPI state is accessible
                if hasattr(self.iracing_api, 'state'):
                    track_length = self.iracing_api.state.get('track_length', 0)
                    if track_length > 0:
                        logger.debug(f"get_track_length from iracing_api.state: {track_length}")
                        return float(track_length)
                ...
                # Try to access ir_sdk.state directly if SimpleIRacingAPI exposes it
                if hasattr(self.iracing_api, 'ir_sdk') and hasattr(self.iracing_api.ir_sdk, 'state'):
                    track_length = self.iracing_api.ir_sdk.state.get('track_length', 0)
                    if track_length > 0:
                        logger.debug(f"get_track_length from iracing_api.ir_sdk.state: {track_length}")
                        return float(track_length)
    ...
    # ----- ThreadedDataRetriever Signal Handlers -----
    def _on_telemetry_fetched(self, lap_id, data, message):
        """Handle telemetry data fetched from the background thread."""
        logger.info(f"Received telemetry data for lap {lap_id}: {message}")
        
        # Determine which lap this is for (left or right)
        left_lap_id = self.left_lap_combo.currentData()
        right_lap_id = self.right_lap_combo.currentData() if hasattr(self, 'right_lap_combo') else None
        
        # For single lap view
        if left_lap_id == lap_id and (right_lap_id is None or not hasattr(self, 'comparison_mode') or not self.comparison_mode):
            # Process for single lap view - create dict structure expected by load_throttle_data
            track_length = self.get_track_length()
            track_context = {}  # Context could be added later if needed
            
            # Format for graph update - the data is already a list of points
            data_dict = {"points": data}
            
            # Update the graphs
            if hasattr(self, 'throttle_graph'): 
                self.throttle_graph.update_graph(data_dict, track_length, track_context)
            if hasattr(self, 'brake_graph'): 
                self.brake_graph.update_graph(data_dict, track_length, track_context)
            if hasattr(self, 'steering_graph'): 
                self.steering_graph.update_graph(data_dict, track_length, track_context)
            if hasattr(self, 'speed_graph'): 
                self.speed_graph.update_graph(data_dict, track_length, track_context)
                
            # Process and show any warnings about data completeness
            if data:
                is_incomplete, stopped_sections, log_msg = self._check_lap_completeness(data, track_length, lap_id)
                if log_msg: 
        ...
        # For comparison view - handling left lap
        elif left_lap_id == lap_id and hasattr(self, 'comparison_mode') and self.comparison_mode:
            # Store the data for later comparison when both laps are loaded
            self._left_lap_data = data
            # Check if we already have right lap data
            if hasattr(self, '_right_lap_data') and self._right_lap_data is not None:
                self._process_comparison_data()
        
        # For comparison view - handling right lap
        elif right_lap_id == lap_id and hasattr(self, 'comparison_mode') and self.comparison_mode:
            # Store the data for later comparison when both laps are loaded
            self._right_lap_data = data
            # Check if we already have left lap data
            if hasattr(self, '_left_lap_data') and self._left_lap_data is not None:
                self._process_comparison_data()

    def _process_comparison_data(self):
        """Process and display comparison data when both laps are loaded."""
        if not hasattr(self, '_left_lap_data') or not hasattr(self, '_right_lap_data'):
            return
        
        left_data = {"points": self._left_lap_data}
        right_data = {"points": self._right_lap_data}
        
        track_length = self.get_track_length()
        
        # Update the comparison graphs
        if hasattr(self, 'throttle_graph'):
        ...
        # Clean up temp data
            del self._left_lap_data
            del self._right_lap_data
    ...
    def _on_laps_fetched(self, session_id, data, message):
        """Handle laps data fetched from the background thread."""
        logger.info(f"Received {len(data) if data else 0} laps for session {session_id}: {message}")
        
        # Update the lap combo boxes
        if hasattr(self, 'left_lap_combo'):
            self.left_lap_combo.clear()
            self.left_lap_combo.addItem("Select Lap (A)", None)
            
        if hasattr(self, 'right_lap_combo'):
            self.right_lap_combo.clear()
            self.right_lap_combo.addItem("Select Lap (B)", None)
        ...
        sorted_laps = sorted(data, key=lambda x: x.get('lap_number', 0))
        ...
        # Populate lap combo boxes
        for index, lap in enumerate(sorted_laps):
            lap_id = lap.get('id')
            lap_num = lap.get('lap_number', 0)
            lap_time = lap.get('lap_time', 0)
            is_valid = lap.get('is_valid', True)
            
            # Format lap display text
            display_text = self._format_lap_display(lap_num, lap_time)
            ...
            if lap_id == current_left_id:
                new_left_index = index + 1  # +1 because we added "Select Lap (A)" at index 0
            if lap_id == current_right_id:
                new_right_index = index + 1  # +1 because we added "Select Lap (B)" at index 0
    ...
    def _on_sessions_fetched(self, data, message):
        """Handle sessions data fetched from the background thread."""
        logger.info(f"Received {len(data) if data else 0} sessions: {message}")
        
        # Store current selection to try and restore it
        current_id = self.session_combo.currentData() if hasattr(self, 'session_combo') else None
        new_index_to_select = 0
        
        # Update the session combo box
        if hasattr(self, 'session_combo'):
            self.session_combo.clear()
            ...
            for index, session in enumerate(sorted_sessions):
                ...
                display_text = f"{track_name} - {car_name}"
                if date_str:
                ...
                if session_id == current_id:
                    new_index_to_select = index + 1  # +1 because we added "Select Session" at index 0
            
            # Try to restore previous selection
            if self.session_combo.count() > new_index_to_select:
                self.session_combo.setCurrentIndex(new_index_to_select)
            elif self.session_combo.count() > 0:
                self.session_combo.setCurrentIndex(0)

    def _on_data_fetch_error(self, operation_type, details, message):
        """Handle errors from the threaded data retriever."""
        logger.error(f"Data fetch error: {operation_type} - {details} - {message}")
        
        # Handle based on operation type
        if operation_type == 'telemetry':
            # Clear graphs if telemetry fetch failed
            empty_data = {"points": []}
            track_length = self.get_track_length()
            if hasattr(self, 'throttle_graph'): 
                self.throttle_graph.update_graph(empty_data, track_length, {})
            if hasattr(self, 'brake_graph'): 
                self.brake_graph.update_graph(empty_data, track_length, {})
            if hasattr(self, 'steering_graph'): 
                self.steering_graph.update_graph(empty_data, track_length, {})
            if hasattr(self, 'speed_graph'): 
                self.speed_graph.update_graph(empty_data, track_length, {})
                
            # Show error in status label if it exists
            if hasattr(self, 'graph_status_label'):
                self.graph_status_label.setText(f"⚠️ Error: {message}")
                self.graph_status_label.setStyleSheet("color: #FF0000; font-weight: bold;")
                self.graph_status_label.setVisible(True)
                
            # If this was for comparison, re-enable button
            if hasattr(self, 'comparison_mode') and self.comparison_mode:
                if hasattr(self, 'compare_button'):
                    self.compare_button.setEnabled(True)
                    self.compare_button.setText("Compare Laps")
        ...
        elif operation_type == 'laps':
            # Clear lap combo boxes
            if hasattr(self, 'left_lap_combo'):
                self.left_lap_combo.clear()
