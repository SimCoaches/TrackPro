...
class SteeringGraphWidget(GraphBase):
    ...
    def update_graph_comparison(self, lap_data_a, lap_data_b, track_length=0, metadata=None):
        """Update the graph with comparison data for two laps."""
        logger.info(f"Updating steering graph with comparison data. Track length: {track_length}m")
        self.comparison_mode = True
        
        # Process metadata if provided (like track corners, markers, etc.)
        if metadata:
        ...
        # Validate data
        valid_a, cleaned_a, debug_a = self.validate_telemetry_data(lap_data_a, 'steering')
        valid_b, cleaned_b, debug_b = self.validate_telemetry_data(lap_data_b, 'steering')
        
        # Log the validation results
        logger.info(f"Lap A steering data validation: {valid_a} ({debug_a['valid_points']}/{debug_a['original_points']} points valid)")
        logger.info(f"Lap B steering data validation: {valid_b} ({debug_b['valid_points']}/{debug_b['original_points']} points valid)")
        
        # Check if we have valid data for either lap
        if not valid_a and not valid_b:
            logger.error("No valid steering data for either lap")
            self.display_message("No valid steering data available for comparison")
            return
            
        # Process the data only if we have valid data for at least one lap
        track_length_a = lap_data_a.get('track_length', 0) if lap_data_a else 0
        track_length_b = lap_data_b.get('track_length', 0) if lap_data_b else 0
            
        # Use provided track length or try to get from either lap
        if track_length <= 0:
            if track_length_a > 0:
                track_length = track_length_a
            elif track_length_b > 0:
                track_length = track_length_b
            else:
                # Default to 1000m if we still don't have a valid track length
                track_length = 1000.0
                logger.warning("No valid track length provided, using default of 1000m")
        
        # Clear any message text
        if hasattr(self, 'message_text') and self.message_text is not None:
            self.message_text.setVisible(False)
        
        # Update the plot title to show comparison info
        self.plot_widget.setTitle("Steering Comparison")
        
        # Resample the data for Lap A
        if valid_a:
            resampled_a = self.preprocess_telemetry_data(
                cleaned_a, track_length, ['Steering'], resolution=self.distance_resolution
            )
        else:
            resampled_a = None
            
        # Resample the data for Lap B
        if valid_b:
            resampled_b = self.preprocess_telemetry_data(
                cleaned_b, track_length, ['Steering'], resolution=self.distance_resolution
            )
        else:
            resampled_b = None
            
        # Update x-axis range if we have valid data
        if resampled_a or resampled_b:
            self.plot_widget.setXRange(0, track_length, padding=0.05)
            
        # Update curves with resampled data
        if resampled_a:
            self.steering_curve.setData(
                resampled_a['x_m'],
                resampled_a['Steering'],
                pen=pg.mkPen(color=self.color_a, width=2),
                name="Lap A Steering"
            )
            self.steering_curve.setVisible(True)
        else:
            # Clear Lap A curve if no valid data
            self.steering_curve.setData([], [])
            
        if resampled_b:
            self.steering_curve_b.setData(
                resampled_b['x_m'],
                resampled_b['Steering'],
                pen=pg.mkPen(color=self.color_b, width=2),
                name="Lap B Steering"
            )
            self.steering_curve_b.setVisible(True)
        else:
            # Clear Lap B curve if no valid data
            self.steering_curve_b.setData([], [])
            
        # Update the legend
        self.update_legend_items(
            valid_a_data=valid_a, 
            valid_b_data=valid_b, 
            lap_a_name=lap_data_a.get('lap_name', 'Lap A') if lap_data_a else 'Lap A',
            lap_b_name=lap_data_b.get('lap_name', 'Lap B') if lap_data_b else 'Lap B'
        )
        
        # Update background areas if track data is available
        if self.metadata and 'corners' in self.metadata:
            self.draw_track_areas(self.metadata['corners'], track_length)
            
        # Force update to ensure curves are displayed correctly
        self.plot_widget.update()
        
        logger.info("Steering comparison graph updated successfully")
