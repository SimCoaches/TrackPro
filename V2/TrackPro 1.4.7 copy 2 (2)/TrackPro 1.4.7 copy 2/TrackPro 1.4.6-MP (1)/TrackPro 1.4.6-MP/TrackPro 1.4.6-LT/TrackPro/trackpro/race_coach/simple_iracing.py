...
from .threaded_lap_saver import ThreadedLapSaver
...
class SimpleIRacingAPI(QObject):
    ...
    def process_telemetry(self):
        ...
        try:
            ...
            if telemetry.get('LapDistPct') is not None and telemetry.get('SessionTimeSecs') is not None:
                ...
                if self.lap_saver:
                    try:
                        # The threaded lap saver will handle lap detection and async saving
                        ...
                        logger.error(f"Error processing telemetry with threaded lap saver: {e}")
    ...
    def disconnect(self):
        ...
        # Clean up the threaded lap saver if it exists
        if hasattr(self, 'lap_saver') and self.lap_saver and hasattr(self.lap_saver, 'shutdown'):
            try:
                logger.info("Shutting down threaded lap saver...")
                self.lap_saver.shutdown()
            except Exception as e:
                logger.error(f"Error shutting down threaded lap saver: {e}")
    ...
    def set_lap_saver(self, lap_saver):
        ...
        """
        # Create a threaded wrapper around the lap saver
        threaded_saver = ThreadedLapSaver(lap_saver)
        self.lap_saver = threaded_saver
        ...
        logger.info("Threaded lap saver set up for telemetry processing")
