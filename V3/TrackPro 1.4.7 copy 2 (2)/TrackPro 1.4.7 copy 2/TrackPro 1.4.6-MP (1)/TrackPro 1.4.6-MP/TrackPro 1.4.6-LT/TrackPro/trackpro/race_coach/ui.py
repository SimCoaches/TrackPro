        ...
        try:
            ...
            if session_ids:
                try:
                    ...
                    laps_query = (
                        main_supabase.client.table("laps").select("session_id").in_("session_id", session_ids).execute()
                    )
...
class CourseCard(QFrame):
    ...
    def create_thumbnail(self, course_data):
        ...
        try:
            ...
            darker_color = QColor(
                int(base_color.red() * 0.7), int(base_color.green() * 0.7), int(base_color.blue() * 0.7)
            )
...
class RaceCoachWidget(QWidget):
        ...
        if self.iracing_api is not None:
            try:
                ...
                # Fall back to IRacingAPI method names (This path likely won't be used anymore)
                    ...
                    logger.warning(
                        "Legacy IRacingAPI callback registration attempted - this might not work as expected."
                    )
                else:
    ...
    def on_save_lap_clicked(self):
        ...
        if not is_valid:
            QMessageBox.warning(
                self, "Incomplete Lap Data", f"This lap has incomplete telemetry data and cannot be saved:\n\n{message}"
            )
        ...
        if hasattr(self, "session_info") and self.session_info:
            track_name = self.session_info.get("current_track", self.session_info.get("TrackName", track_name))
            car_name = self.session_info.get("current_car", self.session_info.get("CarName", car_name))
    ...
    def on_save_lap_finished(self, success, message, diagnostics):
        ...
        if success:
            ...
            QMessageBox.information(
                self, "Lap Saved", f"Lap data saved successfully.\n\nLap ID: {lap_id}\nTelemetry Points: {points_saved}"
            )
            ...
            QMessageBox.critical(self, "Save Failed", f"Failed to save lap data:\n\n{message}")
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            if (
                hasattr(self, "telemetry_monitor_thread")
                and self.telemetry_monitor_thread
                and self.telemetry_monitor_thread.isRunning()
            ):
    ...
    def _on_initial_laps_loaded(self, laps, message):
        ...
        for lap in laps:
            ...
            lap_type = lap.get("lap_type", "")

            # Get lap state from metadata if not directly in the lap record
            if not lap_type and "metadata" in lap:
                try:
                    metadata = json.loads(lap.get("metadata", "{}"))
                    lap_type = metadata.get("lap_type", "")
                except Exception:
                    pass

            # Use the new lap display formatting function with lap_type
            display_text = self._format_lap_display(lap_num, lap_time, lap_type)
    ...
    def refresh_lap_list(self, session_id):
        ...
        try:
            ...
            if response and hasattr(response, "data"):
                ...
                logger.warning(
                    f"Failed to fetch laps for session {session_id} using main app client. Response: {response}"
                )
    ...
    def on_compare_clicked(self):
        ...
        if (
            hasattr(self, "telemetry_fetch_thread")
            and self.telemetry_fetch_thread is not None
        ...
        if (
            hasattr(self, "telemetry_load_thread")
            and self.telemetry_load_thread
            and self.telemetry_load_thread.isRunning()
        ):
        ...
        self.telemetry_fetch_worker.finished.connect(
            self.telemetry_fetch_worker.deleteLater
        )  # Schedule worker deletion
        self.telemetry_fetch_thread.finished.connect(
            self.telemetry_fetch_thread.deleteLater
    ...
    def _on_telemetry_fetch_finished(self, left_data, right_data):
        ...
        if success_left:
            ...
            left_throttle_avg = (
                sum(point.get("Throttle", point.get("throttle", 0)) for point in left_points) / len(left_points) * 100
                if left_points
                else 0
            )
            left_brake_avg = (
                sum(point.get("Brake", point.get("brake", 0)) for point in left_points) / len(left_points) * 100
                if left_points
                else 0
            )
        ...
        if success_right:
            ...
            right_throttle_avg = (
                sum(point.get("Throttle", point.get("throttle", 0)) for point in right_points) / len(right_points) * 100
                if right_points
                else 0
            )
            right_brake_avg = (
                sum(point.get("Brake", point.get("brake", 0)) for point in right_points) / len(right_points) * 100
                if right_points
                else 0
            )
        ...
        def ensure_lap_dist_field(lap_data):
            ...
            for point in lap_data["points"]:
                if "LapDist" not in point and "track_position" in point:
                    ...
                    point["LapDist"] = (
                        point["track_position"] * track_length if track_length > 0 else point["track_position"] * 1000
                    )
        ...
        def preprocess_lap_data(lap_data):
            ...
            for point in lap_data.get("points", []):
                ...
                try:
                    ...
                    # Add processed point
                    valid_points.append(
                        {"LapDist": dist_m, "throttle": throttle, "brake": brake, "steering": steering, "speed": speed}
                    )
                except (ValueError, TypeError):
            ...
            logger.info(
                f"Preprocessed lap data: {len(valid_points)} valid points from {len(lap_data.get('points', []))} raw points"
            )
    ...
    def _debug_check_curve_state(self):
        ...
        try:
            ...
            throttle_a_points = (
                len(self.throttle_graph.throttle_curve.xData)
                if self.throttle_graph.throttle_curve.xData is not None
                else 0
            )
            throttle_b_points = (
                len(self.throttle_graph.throttle_curve_b.xData)
                if self.throttle_graph.throttle_curve_b.xData is not None
                else 0
            )
            ...
            steering_a_points = (
                len(self.steering_graph.steering_curve.xData)
                if self.steering_graph.steering_curve.xData is not None
                else 0
            )
            steering_b_points = (
                len(self.steering_graph.steering_curve_b.xData)
                if self.steering_graph.steering_curve_b.xData is not None
                else 0
            )
            ...
            speed_a_points = (
                len(self.speed_graph.speed_curve.xData) if self.speed_graph.speed_curve.xData is not None else 0
            )
            speed_b_points = (
                len(self.speed_graph.speed_curve_b.xData) if self.speed_graph.speed_curve_b.xData is not None else 0
            )
    ...
    def _on_telemetry_fetch_error(self, msg_left, msg_right):
        ...
        QMessageBox.critical(
            self, "Telemetry Error", f"Could not load telemetry data.\n\nLap A: {msg_left}\nLap B: {msg_right}"
        )
    ...
    def _on_telemetry_load_finished(self, loaded_lap_id, data_to_plot, track_length, track_context):
        ...
        logger.info(f"Telemetry data loading completed successfully for lap {loaded_lap_id}")
        ...
        if data_to_plot and data_to_plot.get("points"):
            ...
            is_incomplete, stopped_sections, log_msg = self._check_lap_completeness(points, track_length, loaded_lap_id)
            if log_msg:
            ...
            if hasattr(self, "graph_status_label"):
                if is_incomplete:
                    ...
                    if stopped_sections:
                        stops_text = (
                            f"{len(stopped_sections)} location(s)" if len(stopped_sections) > 1 else "1 location"
                        )
    ...
    def _handle_lap_selection_change(self, left_lap_id, right_lap_id):
        ...
        can_compare = (
            left_lap_id
            and right_lap_id
            and left_lap_id != "ALL_LAPS"
            and right_lap_id != "ALL_LAPS"
            and left_lap_id != right_lap_id
        )
        ...
        if can_compare:
            ...
            logger.info(
                f"Lap selection changed. Comparison not possible (Left: {left_lap_id}, Right: {right_lap_id}). Trying single view for Lap A."
            )
            ...
            if hasattr(self.throttle_graph, "throttle_curve_b"):
                self.throttle_graph.throttle_curve_b.setData([], [])
            if hasattr(self.brake_graph, "brake_curve_b"):
                self.brake_graph.brake_curve_b.setData([], [])
            if hasattr(self.steering_graph, "steering_curve_b"):
                self.steering_graph.steering_curve_b.setData([], [])
            if hasattr(self.speed_graph, "speed_curve_b"):
                self.speed_graph.speed_curve_b.setData([], [])
            # Reset titles
    ...
    def load_throttle_data(self, lap_id: str):
        ...
        if (
            ...
            if worker_lap_id and worker_lap_id != lap_id:
                ...
                logger.info(
                    f"Request to load lap {lap_id} which is already in the process of loading. Current request will be ignored if it finishes first."
                )
    ...
    def _on_telemetry_load_finished(self, loaded_lap_id, data_to_plot, track_length, track_context):
        ...
        if not hasattr(self, "_current_requested_lap_id") or self._current_requested_lap_id != loaded_lap_id:
            logger.warning(
                f"Received telemetry data for STALE lap {loaded_lap_id} (expected {getattr(self, '_current_requested_lap_id', 'None')}). Discarding."
            )
        ...
        if data_to_plot and data_to_plot.get("points"):
            points = data_to_plot.get("points", [])
            is_incomplete, stopped_sections, log_msg = self._check_lap_completeness(
                points, track_length, loaded_lap_id
            )  # Use loaded_lap_id
            if log_msg:
                logger.warning(log_msg)
    ...
    def _fetch_track_context(self, client, track_id, track_length):
        ...
        try:
            # Corners
            corner_result = (
                client.table("track_corners")
                .select("track_position,corner_number")
            ...
            # Markers
            marker_result = (
                client.table("track_markers").select("track_position,label").eq("track_id", track_id).execute()
            )
            ...
            logger.info(
                f"Fetched track context for {track_id}: {len(context.get('corners',[]))} corners, {len(context.get('markers',[]))} markers"
            )
    ...
    def load_all_laps_for_session(self, session_id):
        ...
        try:
            ...
            laps_result = (
                client.table("laps").select("id,lap_number").eq("session_id", session_id).order("lap_number").execute()
            )

            if not laps_result.data:
                logger.warning(f"No laps found for session {session_id}")
                if hasattr(self, "throttle_graph"):
                    self.throttle_graph.update_graph([], track_length, track_context)
                if hasattr(self, "graph_status_label"):
    ...
    def test_authentication(self):
        ...
        if not is_logged_in:
            QMessageBox.warning(self, "Authentication Test", "Not logged in. Please log in to access telemetry data.")
        elif not has_client_session:
            QMessageBox.warning(
                self, "Authentication Test", "Logged in but client session not established. Try restarting the app."
            )
        else:
            QMessageBox.information(self, "Authentication Test", "Logged in and client session is active.")
    ...
    def refresh_supabase_auth(self):
        ...
        try:
            ...
            if app_client:
                ...
                logger.info(
                    f"Main client auth status: {app_client.is_authenticated() if hasattr(app_client, 'is_authenticated') else 'unknown'}"
                )
        except Exception as e:
        ...
        try:
            ...
            if hasattr(Supabase.client, "supabase") and Supabase.client.supabase:
                ...
                if token:
                    try:
                        if hasattr(Supabase.client.supabase, "auth") and hasattr(
                            Supabase.client.supabase.auth, "set_session"
                        ):
    ...
    def _on_right_lap_changed(self, index):
        ...
        if left_lap_id and right_lap_id and left_lap_id != "ALL_LAPS" and right_lap_id != "ALL_LAPS":
    ...
    def save_partial_laps(self, lap_saver):
        ...
        try:
            ...
            if result == QMessageBox.Yes:
                ...
                # Show confirmation
                QMessageBox.information(self, "Laps Saved", "Partial laps have been force-saved.")
        except Exception as e:
...
class TelemetryLoadWorker(QObject):
    ...
    def _fetch_track_context(self, client, track_id, track_length):
        ...
        try:
            ...
            if self.is_cancelled:
                return context  # Added this check
            ...
            marker_result = (
                client.table("track_markers").select("track_position,name").eq("track_id", track_id).execute()
            )
            if self.is_cancelled:
                return context
    ...
    def run(self):
        ...
        try:
            ...
            if client:  # Only try to fetch context if client is available
                ...
                if lap_data_for_context:
                    try:
                        ...
                        if session_id:
                            session_result = (
                                client.table("sessions").select("track_id").eq("id", session_id).single().execute()
                            )
                            if session_result.data:
                                track_id = session_result.data.get("track_id")
                                if track_id:
            ...
            if valid_points:
                ...
                logger.info(
                    f"TelemetryLoadWorker successfully processed {len(valid_points)} points for lap {self.lap_id}"
                )
            else:
